...
import numpy as np
import pandas as pd
# from models.gruae_arch import GRUAE  # OR paste class here
...
class GRUAE(nn.Module):
    ...
    def forward(self, x):
        ...
        dec_init = self.act(self.dec_fc(latent)) \
            .unsqueeze(0).repeat(self.layers, 1, 1)

        dec_input = dec_init[-1] \
...
model.load_state_dict(
    torch.load("models/gruae_model_v2.pt", map_location=DEVICE)
)
model.eval()
